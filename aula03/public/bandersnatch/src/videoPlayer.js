class VideoMediaPlayer {
    ...
    initializeCodec() {
        ...
        const mediaSourceSupporter = !!window.MediaSource

        if (!mediaSourceSupporter) {
        ...
        if (!codecSupported) {
            alert(`Your browser does not support the codec: ${this.manifestJSON.codec}`)
            return;
        }
    ...
    async processBufferSegments(allSegments) {
        ...
        return new Promise((resolve, reject) => {
            const updatedEnd = (_) => {
                sourceBuffer.removeEventListener('updateend', updatedEnd)
                sourceBuffer.timestampOffset = this.videoDuration

                return resolve()
            }

            sourceBuffer.addeventListener('updateend', updatedEnd)
